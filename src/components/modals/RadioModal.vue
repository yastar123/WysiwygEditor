  ...
  <div class="modal-backdrop" @click.self="close">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button class="btn btn-primary" @click="insertRadio">Insert</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    // Insert the radio button group into the editor
    const insertRadio = () => {
      ...
      // Create a unique name for the radio group
      const radioGroupName = `radio_${Date.now()}`;
      
      // Create radio button HTML
      let radioHtml = `
        <div class="editor-element" data-type="radio" data-correct="${correctAnswer.value}">
          ...
          <div class="radio-group">
            ${options.value.map(opt => `
              <div class="radio-option">
                <input type="radio" name="${radioGroupName}" value="${opt}" id="${radioGroupName}_${opt.replace(/\s+/g, '_')}">
                <label for="${radioGroupName}_${opt.replace(/\s+/g, '_')}">${opt}</label>
              </div>
            `).join('')}
          </div>
      ...
      if (selection) {
        ...
        tempDiv.innerHTML = radioHtml.trim();
